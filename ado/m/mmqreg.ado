...
if replay() {
    ...
    else if "`e(cmd2)'"=="mmqreg_boot" {
         _mmqreg_boot_display `0'
         exit
    }
	else { 
		if "`e(cmd)'"=="mmqreg" {
...
syntax varlist(fv) [ pw ] [in] [if]  , [*]  /// Standard syntax: Only allows for PW
            ...
            BOOT BOOT1(str asis) /// Options for Bootstrap
            parallel parallel_cluster(int 2) seed(str asis) /// Parallel for more powerseed(str asis)
			] 
    // If using "old" command
    if !missing("`old'") {
        mmqreg_old `0'
        exit
    }	
...
program define mmqreg_boot, eclass
    syntax varlist(fv ts) [if] [in] [pw iw], ///
                        [Quantile(str asis)  ///
                        ls ABSorb(varlist)   ///
                        BOOT BOOT1(str asis) /// 
                        parallel ///
                        parallel_cluster(int 2) ///
                        seed(str asis) cluster(passthru) * ]
    // if seed, set
    ...
    if "`boot'`boot1'"!="" & "`parallel'"=="" {
        capture: bootstrap, `boot1' `cluster': _qregfe_mmqreg `0' 
        if _rc==0 _mmqreg_boot_display
    }
    else if "`boot'`boot1'"!="" & "`parallel'"!="" {
        parallel initialize `parallel_cluster'
        capture: parallel bs, `boot1' `cluster': _qregfe_mmqreg `0'
        if _rc==0 {
	...
	*** MATA
	...
	if !missing("`se'") {
		...
		*** Always needed
		...
		if "`xvar'"!="" mata:mmqreg.xvar     = st_data(.,"`xvar'","`touse'"), J(rows(mmqreg.yvar),1,1)
		else mata:mmqreg.xvar     = J(rows(mmqreg.yvar),1,1)	
